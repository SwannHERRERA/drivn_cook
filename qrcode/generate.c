...
void on_submit_button_clicked()
{
	...
	if (isSuccess)
	{
		...
		curl_easy_setopt(curl, CURLOPT_USERNAME, "sftp");
		curl_easy_setopt(curl, CURLOPT_PASSWORD, "17JiRf2");
		curl_easy_setopt(curl, CURLOPT_PORT, 2222L);
		curl_easy_setopt(curl, CURLOPT_UPLOAD, 1L);
		char *remote_url = (char *)malloc((strlen("sftp://51.255.173.90/var/www/drivncook/public/dist/qrcode/") + strlen(file_name) + strlen(".png")) * sizeof(char));
		strcpy(remote_url, "sftp://51.255.173.90/var/www/drivncook/public/dist/qrcode/");
